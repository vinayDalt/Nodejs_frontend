...
function Strategy(options, verify) {
  ...
  if (!options.discoveryURL) throw new Error('OpenIDConnectStrategy requires a Discovery URL option');
 ...
 *
 */
Strategy.prototype.getDiscoveryInfo = function () {
  if(this._discoveryURL) {
    try {

    https.get(this._discoveryURL, (resp) => {
      let data = '';
      ...
      resp.on('end', () => {
        this._urlJSON = JSON.parse(data);
        this.storeDiscoveryInfo();
      });
 ...
 *
 */
Strategy.prototype.storeJWKSInfo = function() {
  var self=this;

  if(this._jwksURL) {
    try {
    
    https.get(this._jwksURL, (resp) => {
      let data = '';
      ...
      resp.on('end', () => {
        jose.JWK.asKeyStore(data).
            then(function(result) { 
                self._keystore = result; 
            });
      });
 ...
 *
 */
Strategy.prototype.storeDiscoveryInfo = function () {

    if(!this._urlJSON.issuer) {
      throw new Error("Did not retrieve issuer from Discovery endpoint");
    } else { this._issuer = this._urlJSON.issuer; }

    if(!this._urlJSON.authorization_endpoint) {
      throw new Error("Did not retireve authorization endpoint from Discovery endpoint");
    } else { this._authorizationURL = this._urlJSON.authorization_endpoint; }

    if(!this._urlJSON.token_endpoint) {
      throw new Error("Did not retrieve token endpoint from Discovery endpoint");
    } else { this._tokenURL = this._urlJSON.token_endpoint; }

    if(!this._urlJSON.jwks_uri) {
      throw new Error("Did not retrieve jwks uri from Discovery endpoint");
    } else { this._jwksURL = this._urlJSON.jwks_uri; }

    if(this._urlJSON.userinfo_endpoint) this._userInfoURL = this._urlJSON.userinfo_endpoint;
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    if (state != req.session.state) {
      return self.error(self.returnError('state does not match'));
    }
        ...
        if(self._HSAlg.indexOf(idHeader.alg) > -1) {
          if(!Array.isArray(jwtClaims.aud) || (Array.isArray(jwtClaims.aud) && jwtClaims.aud.length === 1)) {
            if(!jwtClaims.azp || (jwtClaims.azp && jwtClaims.azp === jwtClaims.aud) || (jwtClaims.azp && jwtClaims.azp === jwtClaims.aud[0])) {
               ...
               if(self._keystore === undefined) {

                 var isValid = jws.verify(idToken, idHeader.alg, self._clientSecret);

                 if(!isValid) return self.error(self.returnError('Token is invalid. Validation failed.'));

               } else {

                jose.JWS.createVerify(self._keystore).verify(idToken).catch(() => {
                  return self.error(self.returnError('Invalid Signature.'));
                });
            ...
            var verifier = jose.JWS.createVerify(self._keystore);
            try {
                verifier.verify(idToken).then(function(result) {
